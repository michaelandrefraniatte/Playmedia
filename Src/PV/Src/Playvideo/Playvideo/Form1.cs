    ...
    public partial class Form1 : Form
    {
        ...
        private void Form1_Shown(object sender, EventArgs e)
        {
            try
            {
                ...
                string path = @"playvideo.txt";
                string readText = DecryptFiles(path + ".encrypted", "tybtrybrtyertu50727885");
                string folderpath = "file:///" + System.Reflection.Assembly.GetEntryAssembly().Location.Replace(@"file:\", "").Replace(Process.GetCurrentProcess().ProcessName + ".exe", "").Replace(@"\", "/").Replace(@"//", "") + "vid/";
                readText = readText.Replace("file:///C:/Users/mic/Documents/GitHub/playvideo/vid/", folderpath);
                string oldobject = "'Game': ['AnticheatingSolution.mp4', 'MonogameLearning.mp4'], 'Science': []";
        ...
        public static string CreateObject(string targetDirectory)
        {
            ...
            foreach (string subdirectory in subdirectoryEntries)
            {
                ...
                {
                    vid = fileName.Replace(subdirectory, "").Replace(@"\", "");
                    if (vid.EndsWith(".mp4"))
                    {
                        arraycreated += "'" + vid + "', ";
                    }
