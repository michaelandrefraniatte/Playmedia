...
namespace Playbook
{
    public partial class Form1 : Form
    {
        ...
        private void Form1_Shown(object sender, EventArgs e)
        {
            ...
            string path = @"playbook.txt";
            string readText = DecryptFiles(path + ".encrypted", "tybtrybrtyertu50727885");
            string folderpath = "file:///" + System.Reflection.Assembly.GetEntryAssembly().Location.Replace(@"file:\", "").Replace(Process.GetCurrentProcess().ProcessName + ".exe", "").Replace(@"\", "/").Replace(@"//", "") + "ebk/";
            readText = readText.Replace("file:///C:/Users/mic/Documents/GitHub/playbook/ebk/", folderpath);
            string oldobject = "'Game': ['AnticheatingSolution.pdf', 'MonogameLearning.pdf'], 'Science': []";
        ...
        public static string CreateObject(string targetDirectory)
        {
            ...
            foreach (string subdirectory in subdirectoryEntries)
            {
                ...
                {
                    ebk = fileName.Replace(subdirectory, "").Replace(@"\", "");
                    if (ebk.EndsWith(".pdf")) 
                    {
                        arraycreated += "'" + ebk + "', ";
                    }
